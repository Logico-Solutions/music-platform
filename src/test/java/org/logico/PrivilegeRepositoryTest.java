...
public class PrivilegeRepositoryTest {
    ...
    public void setup() {
        ...
        Mockito.when(privilegeRepository.findPaginatedAndSortedById(0, 3)).thenReturn(mockPrivileges);
        Mockito.when(privilegeRepository.findPaginatedAndSortedById(0, 2)).thenReturn(mockPrivileges.subList(0, 2));
        Mockito.when(privilegeRepository.findPaginatedAndSortedById(10, 2)).thenReturn(List.of());
        Mockito.when(privilegeRepository.findPaginatedAndSortedById(1, 1)).thenReturn(List.of(mockPrivileges.get(0)));
        Mockito.when(privilegeRepository.findPaginatedAndSortedById(0, 1)).thenReturn(List.of(mockPrivileges.get(0)));
    }
